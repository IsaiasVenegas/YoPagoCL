...
class ItemAssignment(SQLModel, table=True):
    ...
    creditor: "TableParticipant" = Relationship(
        ...
        sa_relationship_kwargs={
            "foreign_keys": lambda: [ItemAssignment.__table__.c.creditor_id]
        }
    )
    debtor: Optional["TableParticipant"] = Relationship(
        back_populates="debtor_assignments",
        sa_relationship_kwargs={
            "foreign_keys": lambda: [ItemAssignment.__table__.c.debtor_id]
        }
    )
